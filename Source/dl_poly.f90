...
! OPEN FFLUX OUTPUT FILE - BCBS 01/2019

  If (idnode == 0) Then
    
    Open(Unit=nffluxdt, File='FFLUX', Status='replace')
    Write(nffluxdt, '(a)') 'FFLUX OUTPUT'
...
! INITIALISE FFLUX
  ...
  !Store megatm as a FFLUX variable (needed for MPI) - BCBS
  totAtms = megatm

  Call fflux_initialisation

! If printing FFLUX dipoles then create the dipole file.

  If (ldipole .EQV. .true.) Then

    Open(Unit=ndipoledt, File='DIPOLE', Status='replace')

  End If

  If (lfflux_energy) Then
    
    Open(Unit=nenergydt, File='IQA_ENERGIES', Status='replace')

  End If
  If (lfflux_force) Then
  
    Open(Unit=nforcedt, File='IQA_FORCES', Status='replace')
