...
Subroutine ewald_real_mforces &
           (iatm,rcut,alpha,epsq,xxt,yyt,zzt,rrt,engcpe_rl,vircpe_rl,stress,drewd,rdrewd,erc,fer,dQG_da,efx,efy,efz)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  Use setup_module
  Use config_module, Only : natms,ltg,list,fxx,fyy,fzz,nlast
  Use mpoles_module
  Use fflux_module,  Only : max_atms_model
  ...
  Real( Kind = wp ), Dimension(natms),      Intent( InOut ) :: efx,efy,efz
...
!  Real( Kind = wp ), Dimension( : ), Allocatable, Save :: erc,fer
  ...
  !BCBS added to prevent divergence issues with higher order electrostatics
  Real( Kind = wp ) :: convRad
...
!!$OMP PARALLEL DO SCHEDULE(DYNAMIC) DEFAULT(NONE) PRIVATE(m,jatm,jmp,rrr,k,ppp,vk0,vk1,vk2,t1,t2,erfcr,d1,k1,k2,k3,kx,ky,kz,sx,sy,sz,ks1,ks2,ks3,ks11,ks21,ks31,n,alphan,tmp,ii,jj,txyz,engmpl,fx,fy,fz) SHARED(list,iatm,mplgfr,indipx,indipy,indipz,induce,xxt,yyt,zzt,rrt,rcut,rdrewd,mxompl,mplmap,zero_plus,alpha,imp,impx,impy,impz,tix,tiy,tiz,jmpx,jmpy,jmpz,tjx,tjy,tjz,idi,ltg,natms,erc,mximpl,scl,convRad) REDUCTION(+:engcpe_rl,vircpe_rl,fix,fiy,fiz,strs1,strs2,strs3,strs5,strs6,strs9,fxx,fyy,fzz)
     Do m=1,list(0,iatm)
...
! truncation of potential
        ...
        If (Maxval(Abs(jmp)) > zero_plus .and. rrr < rcut .and. rrr > convRad) Then
...
! calculate forces
           ...
           engmpl,fx,fy,fz,iatm,jatm,scl,dQG_da,fac,fix,fiy,fiz,kx,efx,efy,efz)
