Subroutine ewald_real_mforces_d &
           (iatm,rcut,alpha,epsq,xxt,yyt,zzt,rrt,engcpe_rl,vircpe_rl,stress,drewd,rdrewd,alp2,co1,co2,co3,co4,co5,exclcoef,twzz,twtwz,fozz,erc,fer,dQG_da,efx,efy,efz,fxc,fyc,fzc,mpi_counter)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  Use setup_module
  Use config_module, Only : natms,ltg,list,fxx,fyy,fzz,nlast,lsi,lsa
  Use mpoles_module
  Use ewald_module,  Only : engsic
  Use fflux_module,  Only : poleOrder,cutDipole,cutQuad,max_atms_model,alf, &
                          & non_alf_atms,atm_label_alias,conv_B2A,model_list, &
                          & lxx,lyy,lzz,dCxx,dCyy,dCzz,mpi_list
  ...
  Real( Kind = wp ),                        Intent( In    ) :: drewd,rdrewd,alp2,co1,co2,co3,co4,co5,exclcoef,twzz,twtwz,fozz
  Real( Kind = wp ), Dimension( 0:mxgele ), Intent( In    ) :: erc,fer
  Real( Kind = wp ),                        Intent( In    ) :: dQG_da(max_atms_model,nlast,mximpl,3)
  Real( Kind = wp ), Dimension( 1:natms ),  Intent( InOut ) :: efx,efy,efz
!  Logical,           Save :: newjob = .true.
!  Real( Kind = wp ), Save :: drewd,rdrewd,alp2,co1,co2,co3,co4,co5,exclcoef, &
!                             twzz,twtwz,fozz

  Integer           :: fail,idi,jatm,k,m,j,local_index,ind,l,alias
  ...
  Real( Kind = wp ) :: scl,rsq,rrr,engmpl,fix,fiy,fiz,fx,fy,fz,  &
                       ...
                       dQ_dx,dQ_dy,dQ_dz,tmpx,tmpy,tmpz

!  Real( Kind = wp ), Dimension( : ), Allocatable, Save :: erc,fer

  Real( Kind = wp ) :: imp(1:mximpl),jmp(1:mximpl),poti(1:10),potj(1:10), &
                     & potj_dd(3),poti_dd(3),potj_cq,poti_cq,potj_qc(6), &
                     & poti_qc(6),potj_dq(3),poti_dq(3),potj_qd(6),poti_qd(6), &
                     & potj_qq(6),poti_qq(6)
...
!OpenMP Added by BCBS 03/2020

! start of primary loop for forces evaluation

! !$OMP  PARALLEL DEFAULT(NONE) PRIVATE(m,iatm,jatm,jmp,jmp1,jmp2,jmp3,jmp4,jmp5,jmp6,jmp7,jmp8,jmp9,jmp10) &
! !$OMP& PRIVATE(rrr,rsq,ecc,ecd,ecq,edd,edq,eqq,fx,fy,fz,xx,yy,zz,xx2,yy2,zz2,exparr,k,ppp,vk0,vk1,vk2,t1,t2) &
! !$OMP& PRIVATE(b0,b1,b2,b3,b4,b5,ijmp,bijmp,tm1,tm2,tm3,tmpi,tmpj,dpx,dpy,dpz,dpxx,dpyy,dpzz,dpxy,dpxz,dpyz) &
! !$OMP& PRIVATE(dpxxx,dpyyy,dpzzz,dpxxz,dpxyy,dpyyz,dpxzz,dpyzz,dpxyz,tm4,tm5,tm6,thrb2,thrb3,dpxxxx,dpyyyy) &
! !$OMP& PRIVATE(dpzzzz,dpxxxy,dpxxxz,dpxyyy,dpyyyz,dpyzzz,dpxzzz,dpxxyy,dpxxzz,dpyyzz,dpxxyz,dpxyyz,dpxyzz) &
! !$OMP& PRIVATE(tm7,tm8,tm9,tm10,fifb3,thrb4,sixb4,tenb4,xyz,dpxxxxx,dpyyyyy,dpzzzzz,dpxxxxy,dpxxxxz) &
! !$OMP& PRIVATE(dpyyyyz,dpyzzzz,dpxzzzz,dpxyyyy,dpxxxyy,dpxxxzz,dpyyyzz,dpyyzzz,dpxxyyy,dpxxzzz,dpxyyzz) &
! !$OMP& PRIVATE(dpxxyyz,dpxxyzz,dpxyyyz,dpxyzzz,dpxxxyz,tm11,tm12,tm13,tm14,tm15,engmpl,dpxxy) &
! !$OMP& SHARED(list,mplgfr,mxompl,induce,indipx,indipy,indipz,rrt,zero_plus,rcut,xxt,yyt,zzt,alp2,rdrewd,erc,ltg) &
! !$OMP& SHARED(co1,co2,co3,co4,co5,imp,imp1,imp2,imp3,imp4,imp5,imp6,imp7,imp8,imp9,imp10,natms,fxx,fyy,fzz,idi) &
! !$OMP& REDUCTION(+:engcpe_rl,vircpe_rl,strs1,strs2,strs3,strs5,strs6,strs9,fix,fiy,fiz)
...
! get the multipoles for site j and the components for its infinitesimal rotations
        ...
        jmpx1=jmpx(1); jmpy1=jmpy(1); jmpz1=jmpz(1)
...
! There is no torque for charges
           ...
           If ((mxompl == 1) .AND. (rrr < cutDipole)) Then
...
! charge-dipole interactions
              ...
              potj(2) = -1.0_wp*scl*jmp2*dpx
              potj(3) = -1.0_wp*scl*jmp3*dpy
              potj(4) = -1.0_wp*scl*jmp4*dpz
              potj(5) = scl*jmp1*dpx
              potj(6) = scl*jmp1*dpy
              potj(7) = scl*jmp1*dpz
              ...
              poti(3) = imp3*dpy
              poti(4) = imp4*dpz
              poti(5) = -1.0_wp*imp1*dpx
              poti(6) = -1.0_wp*imp1*dpy
              poti(7) = -1.0_wp*imp1*dpz
...
! dipole-dipole interactions
              ...
              potj_dd(1) = -1.0_wp*scl*((jmp2*dpxx)+(jmp3*dpxy)+(jmp4*dpxz))
              potj_dd(2) = -1.0_wp*scl*((jmp2*dpxy)+(jmp3*dpyy)+(jmp4*dpyz))
              potj_dd(3) = -1.0_wp*scl*((jmp2*dpxz)+(jmp3*dpyz)+(jmp4*dpzz))

              poti_dd(1) = -1.0_wp*((imp2*dpxx)+(imp3*dpxy)+(imp4*dpxz))
              poti_dd(2) = -1.0_wp*((imp2*dpxy)+(imp3*dpyy)+(imp4*dpyz))
              poti_dd(3) = -1.0_wp*((imp2*dpxz)+(imp3*dpyz)+(imp4*dpzz))
              ...
              Do j=1,3
                ...
                If (alf(iatm,j) .EQ. iatm) Then
                  ...
                  dQ_dx = dQG_da(j,iatm,1,1)
                  dQ_dy = dQG_da(j,iatm,1,2)
                  dQ_dz = dQG_da(j,iatm,1,3)

                  efx(iatm) = efx(iatm) - dQ_dx*(potj(1)+potj(2)+potj(3)+potj(4))
                  efy(iatm) = efy(iatm) - dQ_dy*(potj(1)+potj(2)+potj(3)+potj(4))
                  efz(iatm) = efz(iatm) - dQ_dz*(potj(1)+potj(2)+potj(3)+potj(4))

                  !Dipole-charge + dipole-dipole
                  Do l=1,3

                    !Call fflux_cmoment_derivs(iatm,j,l,dQ_dx,dQ_dy,dQ_dz,dQ_da,CartPoles)
                    dQ_dx = dQG_da(j,iatm,l+1,1)
                    dQ_dy = dQG_da(j,iatm,l+1,2)
                    dQ_dz = dQG_da(j,iatm,l+1,3)

                    efx(iatm) = efx(iatm) - dQ_dx*(potj(4+l) + potj_dd(l))
                    efy(iatm) = efy(iatm) - dQ_dy*(potj(4+l) + potj_dd(l))
                    efz(iatm) = efz(iatm) - dQ_dz*(potj(4+l) + potj_dd(l))
                ...
                Else If (alf(iatm,j) <= natms) Then
                  ...
                  efx(alf(iatm,j)) = efx(alf(iatm,j)) - dQ_dx*(potj(1)+potj(2)+potj(3)+potj(4))
                  efy(alf(iatm,j)) = efy(alf(iatm,j)) - dQ_dy*(potj(1)+potj(2)+potj(3)+potj(4))
                  efz(alf(iatm,j)) = efz(alf(iatm,j)) - dQ_dz*(potj(1)+potj(2)+potj(3)+potj(4))
                  ...
                  Do l=1,3
                    ...
                    efx(alf(iatm,j)) = efx(alf(iatm,j)) - dQ_dx*(potj(4+l)+potj_dd(l))
                    efy(alf(iatm,j)) = efy(alf(iatm,j)) - dQ_dy*(potj(4+l)+potj_dd(l))
                    efz(alf(iatm,j)) = efz(alf(iatm,j)) - dQ_dz*(potj(4+l)+potj_dd(l))
                ...
                Else
                  Do k=1,mpi_counter
                    If (alf(iatm,j) .EQ. mpi_list(k)) Then
                      ...
                      fxc(k) = fxc(k) - dQ_dx*(potj(1)+potj(2)+potj(3)+potj(4))
                      fyc(k) = fyc(k) - dQ_dy*(potj(1)+potj(2)+potj(3)+potj(4))
                      fzc(k) = fzc(k) - dQ_dz*(potj(1)+potj(2)+potj(3)+potj(4))
                      ...
                      Do l=1,3
                        ...
                        fxc(k) = fxc(k) - dQ_dx*(potj(4+l)+potj_dd(l))
                        fyc(k) = fyc(k) - dQ_dy*(potj(4+l)+potj_dd(l))
                        fzc(k) = fzc(k) - dQ_dz*(potj(4+l)+potj_dd(l))
                ...
                If ((jatm <= natms) .AND. (alf(jatm,j) <= natms)) Then
                  ...
                  !Charge-charge + charge-dipole
                  dQ_dx = dQG_da(j,jatm,1,1)
                  dQ_dy = dQG_da(j,jatm,1,2)
                  dQ_dz = dQG_da(j,jatm,1,3)
                                                  
                  efx(alf(jatm,j)) = efx(alf(jatm,j)) - dQ_dx*(poti(1)+poti(2)+poti(3)+poti(4))
                  efy(alf(jatm,j)) = efy(alf(jatm,j)) - dQ_dy*(poti(1)+poti(2)+poti(3)+poti(4))
                  efz(alf(jatm,j)) = efz(alf(jatm,j)) - dQ_dz*(poti(1)+poti(2)+poti(3)+poti(4))

                  !Dipole-charge + dipole-dipole
                  Do l=1,3

                    !Call fflux_cmoment_derivs(jatm,j,l,dQ_dx,dQ_dy,dQ_dz,dQ_da,CartPoles)
                    dQ_dx = dQG_da(j,jatm,l+1,1)
                    dQ_dy = dQG_da(j,jatm,l+1,2)
                    dQ_dz = dQG_da(j,jatm,l+1,3)

                    efx(alf(jatm,j)) = efx(alf(jatm,j)) - dQ_dx*(poti(4+l)+poti_dd(l))
                    efy(alf(jatm,j)) = efy(alf(jatm,j)) - dQ_dy*(poti(4+l)+poti_dd(l))
                    efz(alf(jatm,j)) = efz(alf(jatm,j)) - dQ_dz*(poti(4+l)+poti_dd(l))
                ...
                Else If (jatm <= natms) Then
                  Do k=1,mpi_counter
                    If (alf(jatm,j) .EQ. mpi_list(k)) Then
                      ...
                      fxc(k) = fxc(k) - dQ_dx*(poti(1)+poti(2)+poti(3)+poti(4))
                      fyc(k) = fyc(k) - dQ_dy*(poti(1)+poti(2)+poti(3)+poti(4))
                      fzc(k) = fzc(k) - dQ_dz*(poti(1)+poti(2)+poti(3)+poti(4))
                      ...
                      Do l=1,3
                        ...
                        fxc(k) = fxc(k) - dQ_dx*(poti(4+l)+poti_dd(l))
                        fyc(k) = fyc(k) - dQ_dy*(poti(4+l)+poti_dd(l))
                        fzc(k) = fzc(k) - dQ_dz*(poti(4+l)+poti_dd(l))
              ...
              Do j=1,model_list(ltg(iatm))%ptr%natms_model-3
                ...
                If (ind <= natms) Then
                  ...
                  !Charge-charge + charge-dipole
                  dQ_dx = dQG_da(j+3,iatm,1,1)
                  dQ_dy = dQG_da(j+3,iatm,1,2)
                  dQ_dz = dQG_da(j+3,iatm,1,3)
                                                                            
                  efx(ind) = efx(ind) - dQ_dx*(potj(1)+potj(2)+potj(3)+potj(4))
                  efy(ind) = efy(ind) - dQ_dy*(potj(1)+potj(2)+potj(3)+potj(4))
                  efz(ind) = efz(ind) - dQ_dz*(potj(1)+potj(2)+potj(3)+potj(4))

                  !Dipole-charge + dipole-dipole
                  Do l=1,3
                                                             
                    !Call fflux_cmoment_derivs(iatm,j+3,l,dQ_dx,dQ_dy,dQ_dz,dQ_da,CartPoles)
                    dQ_dx = dQG_da(j+3,iatm,l+1,1)
                    dQ_dy = dQG_da(j+3,iatm,l+1,2)
                    dQ_dz = dQG_da(j+3,iatm,l+1,3)

                    efx(ind) = efx(ind) - dQ_dx*(potj(4+l) + potj_dd(l))
                    efy(ind) = efy(ind) - dQ_dy*(potj(4+l) + potj_dd(l))
                    efz(ind) = efz(ind) - dQ_dz*(potj(4+l) + potj_dd(l))
              ...
              Do j=1,model_list(ltg(jatm))%ptr%natms_model-3
                ...
                If ((jatm <= natms) .AND. (ind <= natms)) Then
                  ...
                  !Charge-charge + charge-dipole
                  dQ_dx = dQG_da(j+3,jatm,1,1)
                  dQ_dy = dQG_da(j+3,jatm,1,2)
                  dQ_dz = dQG_da(j+3,jatm,1,3)

                  efx(ind) = efx(ind) - dQ_dx*(poti(1)+poti(2)+poti(3)+poti(4))
                  efy(ind) = efy(ind) - dQ_dy*(poti(1)+poti(2)+poti(3)+poti(4))
                  efz(ind) = efz(ind) - dQ_dz*(poti(1)+poti(2)+poti(3)+poti(4))

                  !Dipole-charge + dipole-dipole
                  Do l=1,3
        
                    !Call fflux_cmoment_derivs(jatm,j+3,l,dQ_dx,dQ_dy,dQ_dz,dQ_da,CartPoles)
                    dQ_dx = dQG_da(j+3,jatm,l+1,1)
                    dQ_dy = dQG_da(j+3,jatm,l+1,2)
                    dQ_dz = dQG_da(j+3,jatm,l+1,3)
                                             
                    efx(ind) = efx(ind) - dQ_dx*(poti(4+l) + poti_dd(l))
                    efy(ind) = efy(ind) - dQ_dy*(poti(4+l) + poti_dd(l))
                    efz(ind) = efz(ind) - dQ_dz*(poti(4+l) + poti_dd(l))
...
! Torques
           ...
           If ((mxompl == 2) .AND. (rrr < cutDipole)) Then
...
! charge-quadrupole interactions
              ...
              potj_cq=scl*((jmp5*dpxx)+(jmp6*dpxy)+(jmp7*dpxz)+(jmp8*dpyy)+(jmp9*dpyz)+(jmp10*dpzz))
              poti_cq=    ((imp5*dpxx)+(imp6*dpxy)+(imp7*dpxz)+(imp8*dpyy)+(imp9*dpyz)+(imp10*dpzz))

              potj_qc(1) = jmp1*scl*dpxx; potj_qc(2) = jmp1*scl*dpxy
              potj_qc(3) = jmp1*scl*dpxz; potj_qc(4) = jmp1*scl*dpyy
              potj_qc(5) = jmp1*scl*dpyz; potj_qc(6) = jmp1*scl*dpzz

              poti_qc(1) = imp1*dpxx; poti_qc(2) = imp1*dpxy
              poti_qc(3) = imp1*dpxz; poti_qc(4) = imp1*dpyy
              poti_qc(5) = imp1*dpyz; poti_qc(6) = imp1*dpzz
...
! dipole-quadrupole interactions
            ...
            If (rrr < cutQuad) Then
              ...
              potj_dq(1)=scl*((jmp5*dpxxx)+(jmp6*dpxxy)+(jmp7*dpxxz)+(jmp8*dpxyy)+(jmp9*dpxyz)+(jmp10*dpxzz))
              potj_dq(2)=scl*((jmp5*dpxxy)+(jmp6*dpxyy)+(jmp7*dpxyz)+(jmp8*dpyyy)+(jmp9*dpyyz)+(jmp10*dpyzz))
              potj_dq(3)=scl*((jmp5*dpxxz)+(jmp6*dpxyz)+(jmp7*dpxzz)+(jmp8*dpyyz)+(jmp9*dpyzz)+(jmp10*dpzzz))

              poti_dq(1)=-1.0_wp*((imp5*dpxxx)+(imp6*dpxxy)+(imp7*dpxxz)+(imp8*dpxyy)+(imp9*dpxyz)+(imp10*dpxzz))
              poti_dq(2)=-1.0_wp*((imp5*dpxxy)+(imp6*dpxyy)+(imp7*dpxyz)+(imp8*dpyyy)+(imp9*dpyyz)+(imp10*dpyzz))
              poti_dq(3)=-1.0_wp*((imp5*dpxxz)+(imp6*dpxyz)+(imp7*dpxzz)+(imp8*dpyyz)+(imp9*dpyzz)+(imp10*dpzzz))

              potj_qd(1)=-1.0_wp*scl*((jmp2*dpxxx)+(jmp3*dpxxy)+(jmp4*dpxxz))
              potj_qd(2)=-1.0_wp*scl*((jmp2*dpxxy)+(jmp3*dpxyy)+(jmp4*dpxyz))
              potj_qd(3)=-1.0_wp*scl*((jmp2*dpxxz)+(jmp3*dpxyz)+(jmp4*dpxzz))
              potj_qd(4)=-1.0_wp*scl*((jmp2*dpxyy)+(jmp3*dpyyy)+(jmp4*dpyyz))
              potj_qd(5)=-1.0_wp*scl*((jmp2*dpxyz)+(jmp3*dpyyz)+(jmp4*dpyzz))
              potj_qd(6)=-1.0_wp*scl*((jmp2*dpxzz)+(jmp3*dpyzz)+(jmp4*dpzzz))

              poti_qd(1)=((imp2*dpxxx)+(imp3*dpxxy)+(imp4*dpxxz))
              poti_qd(2)=((imp2*dpxxy)+(imp3*dpxyy)+(imp4*dpxyz))
              poti_qd(3)=((imp2*dpxxz)+(imp3*dpxyz)+(imp4*dpxzz))
              poti_qd(4)=((imp2*dpxyy)+(imp3*dpyyy)+(imp4*dpyyz))
              poti_qd(5)=((imp2*dpxyz)+(imp3*dpyyz)+(imp4*dpyzz))
              poti_qd(6)=((imp2*dpxzz)+(imp3*dpyzz)+(imp4*dpzzz))
...
! quadrupole-quadrupole interactions
              ...
              potj_qq(1)=scl*((jmp5*dpxxxx)+(jmp6*dpxxxy)+(jmp7*dpxxxz)+(jmp8*dpxxyy)+(jmp9*dpxxyz)+(jmp10*dpxxzz))
              potj_qq(2)=scl*((jmp5*dpxxxy)+(jmp6*dpxxyy)+(jmp7*dpxxyz)+(jmp8*dpxyyy)+(jmp9*dpxyyz)+(jmp10*dpxyzz))
              potj_qq(3)=scl*((jmp5*dpxxxz)+(jmp6*dpxxyz)+(jmp7*dpxxzz)+(jmp8*dpxyyz)+(jmp9*dpxyzz)+(jmp10*dpxzzz))
              potj_qq(4)=scl*((jmp5*dpxxyy)+(jmp6*dpxyyy)+(jmp7*dpxyyz)+(jmp8*dpyyyy)+(jmp9*dpyyyz)+(jmp10*dpyyzz))
              potj_qq(5)=scl*((jmp5*dpxxyz)+(jmp6*dpxyyz)+(jmp7*dpxyzz)+(jmp8*dpyyyz)+(jmp9*dpyyzz)+(jmp10*dpyzzz))
              potj_qq(6)=scl*((jmp5*dpxxzz)+(jmp6*dpxyzz)+(jmp7*dpxzzz)+(jmp8*dpyyzz)+(jmp9*dpyzzz)+(jmp10*dpzzzz))

              poti_qq(1)= (imp5*dpxxxx)+(imp6*dpxxxy)+(imp7*dpxxxz)+(imp8*dpxxyy)+(imp9*dpxxyz)+(imp10*dpxxzz)
              poti_qq(2)= (imp5*dpxxxy)+(imp6*dpxxyy)+(imp7*dpxxyz)+(imp8*dpxyyy)+(imp9*dpxyyz)+(imp10*dpxyzz)
              poti_qq(3)= (imp5*dpxxxz)+(imp6*dpxxyz)+(imp7*dpxxzz)+(imp8*dpxyyz)+(imp9*dpxyzz)+(imp10*dpxzzz)
              poti_qq(4)= (imp5*dpxxyy)+(imp6*dpxyyy)+(imp7*dpxyyz)+(imp8*dpyyyy)+(imp9*dpyyyz)+(imp10*dpyyzz)
              poti_qq(5)= (imp5*dpxxyz)+(imp6*dpxyyz)+(imp7*dpxyzz)+(imp8*dpyyyz)+(imp9*dpyyzz)+(imp10*dpyzzz)
              poti_qq(6)= (imp5*dpxxzz)+(imp6*dpxyzz)+(imp7*dpxzzz)+(imp8*dpyyzz)+(imp9*dpyzzz)+(imp10*dpzzzz)
              ...
              Do j=1,3
                ...
                If (alf(iatm,j) .EQ. iatm) Then
                  ...
                  efx(iatm) = efx(iatm) - dQ_dx*(potj(1)+potj(2)+potj(3)+potj(4)+potj_cq)
                  efy(iatm) = efy(iatm) - dQ_dy*(potj(1)+potj(2)+potj(3)+potj(4)+potj_cq)
                  efz(iatm) = efz(iatm) - dQ_dz*(potj(1)+potj(2)+potj(3)+potj(4)+potj_cq)
                  ...
                  Do l=1,3
                    ...
                    efx(iatm) = efx(iatm) - dQ_dx*(potj(4+l) + potj_dd(l) + potj_dq(l))
                    efy(iatm) = efy(iatm) - dQ_dy*(potj(4+l) + potj_dd(l) + potj_dq(l))
                    efz(iatm) = efz(iatm) - dQ_dz*(potj(4+l) + potj_dd(l) + potj_dq(l))
                  ...
                  Do l=4,9
                    ...
                    efx(iatm) = efx(iatm) - dQ_dx*(potj_qc(l-3)+potj_qd(l-3)+potj_qq(l-3))
                    efy(iatm) = efy(iatm) - dQ_dy*(potj_qc(l-3)+potj_qd(l-3)+potj_qq(l-3))
                    efz(iatm) = efz(iatm) - dQ_dz*(potj_qc(l-3)+potj_qd(l-3)+potj_qq(l-3))
                ...
                Else If (alf(iatm,j) <= natms) Then
                  ...
                  efx(alf(iatm,j)) = efx(alf(iatm,j)) - dQ_dx*(potj(1)+potj(2)+potj(3)+potj(4)+potj_cq)
                  efy(alf(iatm,j)) = efy(alf(iatm,j)) - dQ_dy*(potj(1)+potj(2)+potj(3)+potj(4)+potj_cq)
                  efz(alf(iatm,j)) = efz(alf(iatm,j)) - dQ_dz*(potj(1)+potj(2)+potj(3)+potj(4)+potj_cq)
                  ...
                  Do l=1,3
                    ...
                    efx(alf(iatm,j)) = efx(alf(iatm,j)) - dQ_dx*(potj(4+l)+potj_dd(l)+potj_dq(l))
                    efy(alf(iatm,j)) = efy(alf(iatm,j)) - dQ_dy*(potj(4+l)+potj_dd(l)+potj_dq(l))
                    efz(alf(iatm,j)) = efz(alf(iatm,j)) - dQ_dz*(potj(4+l)+potj_dd(l)+potj_dq(l))
                  ...
                  Do l=4,9
                    ...
                    efx(alf(iatm,j)) = efx(alf(iatm,j)) - dQ_dx*(potj_qc(l-3)+potj_qd(l-3)+potj_qq(l-3))
                    efy(alf(iatm,j)) = efy(alf(iatm,j)) - dQ_dy*(potj_qc(l-3)+potj_qd(l-3)+potj_qq(l-3))
                    efz(alf(iatm,j)) = efz(alf(iatm,j)) - dQ_dz*(potj_qc(l-3)+potj_qd(l-3)+potj_qq(l-3))
                  ...
                  Do k=1,mpi_counter
                    If (alf(iatm,j) .EQ. mpi_list(k)) Then
                     ...
                     !Charge-charge + charge-dipole + charge-quadrupole                                     
                      ...
                      fxc(k) = fxc(k) - dQ_dx*(potj(1)+potj(2)+potj(3)+potj(4)+potj_cq)
                      fyc(k) = fyc(k) - dQ_dy*(potj(1)+potj(2)+potj(3)+potj(4)+potj_cq)
                      fzc(k) = fzc(k) - dQ_dz*(potj(1)+potj(2)+potj(3)+potj(4)+potj_cq)
                      ...
                      Do l=1,3
                        ...
                        fxc(k) = fxc(k) - dQ_dx*(potj(4+l)+potj_dd(l)+potj_dq(l))
                        fyc(k) = fyc(k) - dQ_dy*(potj(4+l)+potj_dd(l)+potj_dq(l))
                        fzc(k) = fzc(k) - dQ_dz*(potj(4+l)+potj_dd(l)+potj_dq(l))
                      ...
                      Do l=4,9
                        ...
                        fxc(k) = fxc(k) - dQ_dx*(potj_qc(l-3)+potj_qd(l-3)+potj_qq(l-3))
                        fyc(k) = fyc(k) - dQ_dy*(potj_qc(l-3)+potj_qd(l-3)+potj_qq(l-3))
                        fzc(k) = fzc(k) - dQ_dz*(potj_qc(l-3)+potj_qd(l-3)+potj_qq(l-3))
                ...
                If ((jatm <= natms) .AND. (alf(jatm,j) <= natms)) Then
                  ...
                  efx(alf(jatm,j)) = efx(alf(jatm,j)) - dQ_dx*(poti(1)+poti(2)+poti(3)+poti(4)+poti_cq)
                  efy(alf(jatm,j)) = efy(alf(jatm,j)) - dQ_dy*(poti(1)+poti(2)+poti(3)+poti(4)+poti_cq)
                  efz(alf(jatm,j)) = efz(alf(jatm,j)) - dQ_dz*(poti(1)+poti(2)+poti(3)+poti(4)+poti_cq)
                  ...
                  Do l=1,3
                    ...
                    efx(alf(jatm,j)) = efx(alf(jatm,j)) - dQ_dx*(poti(4+l)+poti_dd(l)+poti_dq(l))
                    efy(alf(jatm,j)) = efy(alf(jatm,j)) - dQ_dy*(poti(4+l)+poti_dd(l)+poti_dq(l))
                    efz(alf(jatm,j)) = efz(alf(jatm,j)) - dQ_dz*(poti(4+l)+poti_dd(l)+poti_dq(l))

                  End Do

                  !Quadrupole-charge + quadrupole-quadrupole + quadrupole-quadrupole
                  Do l=4,9
                    ...
                    efx(alf(jatm,j)) = efx(alf(jatm,j)) - dQ_dx*(poti_qc(l-3)+poti_qd(l-3)+poti_qq(l-3))
                    efy(alf(jatm,j)) = efy(alf(jatm,j)) - dQ_dy*(poti_qc(l-3)+poti_qd(l-3)+poti_qq(l-3))
                    efz(alf(jatm,j)) = efz(alf(jatm,j)) - dQ_dz*(poti_qc(l-3)+poti_qd(l-3)+poti_qq(l-3))
                  ...
                  Do k=1,mpi_counter
                    If (alf(jatm,j) .EQ. mpi_list(k)) Then
                     ...
                     !Charge-charge + charge-dipole + charge-quadrupole                                     
                      ...
                      fxc(k) = fxc(k) - dQ_dx*(poti(1)+poti(2)+poti(3)+poti(4)+poti_cq)
                      fyc(k) = fyc(k) - dQ_dy*(poti(1)+poti(2)+poti(3)+poti(4)+poti_cq)
                      fzc(k) = fzc(k) - dQ_dz*(poti(1)+poti(2)+poti(3)+poti(4)+poti_cq)
                      ...
                      Do l=1,3
                        ...
                        fxc(k) = fxc(k) - dQ_dx*(poti(4+l)+poti_dd(l)+poti_dq(l))
                        fyc(k) = fyc(k) - dQ_dy*(poti(4+l)+poti_dd(l)+poti_dq(l))
                        fzc(k) = fzc(k) - dQ_dz*(poti(4+l)+poti_dd(l)+poti_dq(l))
                      ...
                      Do l=4,9
                        ...
                        fxc(k) = fxc(k) - dQ_dx*(poti_qc(l-3)+poti_qd(l-3)+poti_qq(l-3))
                        fyc(k) = fyc(k) - dQ_dy*(poti_qc(l-3)+poti_qd(l-3)+poti_qq(l-3))
                        fzc(k) = fzc(k) - dQ_dz*(poti_qc(l-3)+poti_qd(l-3)+poti_qq(l-3))
              ...
              Do j=1,model_list(ltg(iatm))%ptr%natms_model-3
                ...
                If (ind <= natms) Then
                  ...
                  efx(ind) = efx(ind) - dQ_dx*(potj(1)+potj(2)+potj(3)+potj(4)+potj_cq)
                  efy(ind) = efy(ind) - dQ_dy*(potj(1)+potj(2)+potj(3)+potj(4)+potj_cq)
                  efz(ind) = efz(ind) - dQ_dz*(potj(1)+potj(2)+potj(3)+potj(4)+potj_cq)
                  ...
                  Do l=1,3
                    ...
                    efx(ind) = efx(ind) - dQ_dx*(potj(4+l)+potj_dd(l)+potj_dq(l))
                    efy(ind) = efy(ind) - dQ_dy*(potj(4+l)+potj_dd(l)+potj_dq(l))
                    efz(ind) = efz(ind) - dQ_dz*(potj(4+l)+potj_dd(l)+potj_dq(l))
                  ...
                  Do l=4,9
                    ...
                    efx(ind) = efx(ind) - dQ_dx*(potj_qc(l-3)+potj_qd(l-3)+potj_qq(l-3))
                    efy(ind) = efy(ind) - dQ_dy*(potj_qc(l-3)+potj_qd(l-3)+potj_qq(l-3))
                    efz(ind) = efz(ind) - dQ_dz*(potj_qc(l-3)+potj_qd(l-3)+potj_qq(l-3))
              ...
              Do j=1,model_list(ltg(jatm))%ptr%natms_model-3
                ...
                If ((jatm <= natms) .AND. (ind <= natms)) Then
                  ...
                  efx(ind) = efx(ind) - dQ_dx*(poti(1)+poti(2)+poti(3)+poti(4)+poti_cq)
                  efy(ind) = efy(ind) - dQ_dy*(poti(1)+poti(2)+poti(3)+poti(4)+poti_cq)
                  efz(ind) = efz(ind) - dQ_dz*(poti(1)+poti(2)+poti(3)+poti(4)+poti_cq)
                  ...
                  Do l=1,3
                    ...
                    efx(ind) = efx(ind) - dQ_dx*(poti(4+l)+poti_dd(l)+poti_dq(l))
                    efy(ind) = efy(ind) - dQ_dy*(poti(4+l)+poti_dd(l)+poti_dq(l))
                    efz(ind) = efz(ind) - dQ_dz*(poti(4+l)+poti_dd(l)+poti_dq(l))
                  ...
                  Do l=4,9
                    ...
                    efx(ind) = efx(ind) - dQ_dx*(poti_qc(l-3)+poti_qd(l-3)+poti_qq(l-3))
                    efy(ind) = efy(ind) - dQ_dy*(poti_qc(l-3)+poti_qd(l-3)+poti_qq(l-3))
                    efz(ind) = efz(ind) - dQ_dz*(poti_qc(l-3)+poti_qd(l-3)+poti_qq(l-3))
