Subroutine ewald_spme_mforces(alpha,epsq,engcpe_rc,vircpe_rc,stress,dQG_da,efx,efy,efz)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  Use config_module,  Only : cell,volm,natms,nlast,xxx,yyy,zzz,ltg,lsi,&
                           & lsa,fxx,fyy,fzz
  Use mpoles_module
  Use ewald_module
  Use parallel_fft
  Use fflux_module,   Only : alf,non_alf_atms,max_atms_model,model_list

  Implicit None

  Real( Kind = wp ), Intent( In    ) :: alpha,epsq,dQG_da(max_atms_model,nlast,mximpl,3)
  ...
  Integer              :: fail(1:4), i,j,k,l, jj,kk,ll, jjb,jjt, kkb,kkt, llb,llt, &
                          jjtjjb,counter, k1,k2,k3,s1,s2,s3,ks1,ks2,ks3, mm,nn,a,b,c
...
! FFLUX vars - BCBS
  Real( Kind = wp )    :: ka11,kb22,kc33,tmpj,dQ_dx,dQ_dy,dQ_dz
  Integer              :: h,local_index,ind
...
! compute derivatives of kernel
  ...
  Allocate( ka11_pow(0:mxompl+1) )
  Allocate( kb22_pow(0:mxompl+1) )
  Allocate( kc33_pow(0:mxompl+1) )
                             ...
                             If (Mod(ks2,2) == 0) Then
                                Do s1=0,mxompl-s3-s2
                                   ...
                                   If (Mod(ks1,2) == 0) Then
                                      ...
                                      tmpj = exclcoef*tx*imp(mm)*d1(ks1,ks2,ks3)
...
! FFLUX forces
                                      If (mm == nn) Then
                                        !Loop over ALF
                                        Do h=1,3
                                          dQ_dx = dQG_da(h,i,nn,1)
                                          dQ_dy = dQG_da(h,i,nn,2)
                                          dQ_dz = dQG_da(h,i,nn,3)

                                          efx(alf(i,h)) = efx(alf(i,h)) + 2.0_wp*dQ_dx*tmpj
                                          efy(alf(i,h)) = efy(alf(i,h)) + 2.0_wp*dQ_dy*tmpj
                                          efz(alf(i,h)) = efz(alf(i,h)) + 2.0_wp*dQ_dz*tmpj
                                        End Do
                                        !NON-ALF
                                        Do h=1,model_list(ltg(i))%ptr%natms_model-3

                                          ind =local_index(non_alf_atms(ltg(i),h), nlast,lsi,lsa)

                                          If (ind <= natms) Then
                                            dQ_dx = dQG_da(h+3,i,nn,1)
                                            dQ_dy = dQG_da(h+3,i,nn,2)
                                            dQ_dz = dQG_da(h+3,i,nn,3)

                                            efx(ind) = efx(ind) + 2.0_wp*dQ_dx*tmpj
                                            efy(ind) = efy(ind) + 2.0_wp*dQ_dy*tmpj
                                            efz(ind) = efz(ind) + 2.0_wp*dQ_dz*tmpj
                                          End If
                                      ...
                                      Else
                                        !Loop over ALF
                                        Do h=1,3
                                          !Derivative of (k1,k2,k3)
                                          dQ_dx = dQG_da(h,i,nn,1)
                                          dQ_dy = dQG_da(h,i,nn,2)
                                          dQ_dz = dQG_da(h,i,nn,3)
                                                                                           
                                          efx(alf(i,h)) = efx(alf(i,h)) + dQ_dx*tmpj
                                          efy(alf(i,h)) = efy(alf(i,h)) + dQ_dy*tmpj
                                          efz(alf(i,h)) = efz(alf(i,h)) + dQ_dz*tmpj

                                          !Derivative of (s1,s2,s3)
                                          dQ_dx = dQG_da(h,i,mm,1)
                                          dQ_dy = dQG_da(h,i,mm,2)
                                          dQ_dz = dQG_da(h,i,mm,3)

                                          efx(alf(i,h)) = efx(alf(i,h)) + dQ_dx*tmpi
                                          efy(alf(i,h)) = efy(alf(i,h)) + dQ_dy*tmpi
                                          efz(alf(i,h)) = efz(alf(i,h)) + dQ_dz*tmpi
                                        End Do
                                        !NON-ALF
                                        Do h=1,model_list(ltg(i))%ptr%natms_model-3
                                          ...
                                          If (ind <= natms) Then
                                            ...
                                            efx(ind) = efx(ind) + dQ_dx*tmpj
                                            efy(ind) = efy(ind) + dQ_dy*tmpj
                                            efz(ind) = efz(ind) + dQ_dz*tmpj

                                            !Derivative of (s1,s2,s3)
                                            dQ_dx = dQG_da(h+3,i,mm,1)
                                            dQ_dy = dQG_da(h+3,i,mm,2)
                                            dQ_dz = dQG_da(h+3,i,mm,3)

                                            efx(ind) = efx(ind) + dQ_dx*tmpi
                                            efy(ind) = efy(ind) + dQ_dy*tmpi
                                            efz(ind) = efz(ind) + dQ_dz*tmpi
                                          End If
...
!ADDED BY BCBS FOR OPTIMISATION.
  ...
  Do i=2,mxompl+1
    ka11_pow(i) = ka11_pow(i-1)*ka11
    kb22_pow(i) = kb22_pow(i-1)*kb22
    kc33_pow(i) = kc33_pow(i-1)*kc33
  End Do

  Do a=0,mxompl+1
    Do b=0,mxompl+1
      Do c=0,mxompl+1
        kprod(a,b,c) = ka11_pow(a)*kb22_pow(b)*kc33_pow(c)
      End Do
    End Do
  End Do
...
! calculate atomic forces
  ...
  Call spme_mforces(rcell,scale, ixx,iyy,izz, bsddx,bsddy,bsddz, qqc_local, ixb,ixt, iyb,iyt, izb,izt,efx,efy,efz)
...
Contains
  ...
  Subroutine spme_mforces(rcell,scale, ixx,iyy,izz, bsddx,bsddy,bsddz, qqc_local, ixb,ixt, iyb,iyt, izb,izt,efx,efy,efz)
...
! Define extended ranges for the domain = local + halo slice and allocate
    ...
    Allocate(dqqc( ixdb:ixdt, iydb:iydt, izdb:izdt, 3))
...
!             tix=0.0_wp ; tiy=0.0_wp ; tiz=0.0_wp
             ...
             Call fflux_spme_mforces(i,qqc_domain,dqqc,bsddx,bsddy,bsddz,ixx,iyy,izz,ixdb,ixdt,iydb,iydt,izdb,izdt,fff,scale,rcell,dQG_da,efx,efy,efz)
