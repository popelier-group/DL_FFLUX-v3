Subroutine ewald_spme_mforces_d(alpha,epsq,engcpe_rc,vircpe_rc,stress,dQG_da,efx,efy,efz,mpi_counter,fxc,fyc,fzc)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  Use config_module,  Only : cell,volm,natms,nlast,xxx,yyy,zzz,fxx,fyy,fzz, &
                           & nlast,ltg,lsi,lsa
  Use mpoles_module
  Use ewald_module
  Use parallel_fft
  Use fflux_module,   Only : max_atms_model,alf,non_alf_atms,model_list, &
                           & atm_label_alias,conv_B2A,mpi_list

  Implicit None

  Real( Kind = wp ), Intent( In    ) :: alpha,epsq
  Real( Kind = wp ), Intent( In    ) :: dQG_da(max_atms_model,nlast,mximpl,3)
  Integer,           Intent( In    ) :: mpi_counter
  Real( Kind = wp ), Intent(   Out ) :: engcpe_rc,vircpe_rc
  Real( Kind = wp ), Intent( InOut ) :: stress(1:9)
  Real( Kind = wp ), Intent( InOut ) :: efx(natms),efy(natms),efz(natms),fxc(mpi_counter), &
                                      & fyc(mpi_counter),fzc(mpi_counter)
...
! DaFT arrays local indices
  ...
  Real( Kind = wp )    :: exclcoef,twzz,pot_self(10),dQ_dx,dQ_dy,dQ_dz,tmpx, &
                          tmpy,tmpz,twtwz,fozz,pot_self_cq
  Integer              :: local_index,Ind,alias,atm
...
!FFLUX self forces
  ...
  If (mxompl == 1) Then
    Do i=1,natms
      ...
      pot_self(1) = 2.0_wp*exclcoef*mplgfr(1,i)
      pot_self(2) = 2.0_wp*twzz*mplgfr(2,i)
      pot_self(3) = 2.0_wp*twzz*mplgfr(3,i)
      pot_self(4) = 2.0_wp*twzz*mplgfr(4,i)

      Do j=1,3
    
        If (alf(i,j) .EQ. i) Then

          !Charge-charge (self)
          dQ_dx = dQG_da(j,i,1,1)
          dQ_dy = dQG_da(j,i,1,2)
          dQ_dz = dQG_da(j,i,1,3)
  
          efx(i) = efx(i) + dQ_dx*(pot_self(1))
          efy(i) = efy(i) + dQ_dy*(pot_self(1))
          efz(i) = efz(i) + dQ_dz*(pot_self(1))

          !Dipole-dipole (self)
          Do l=1,3
                                                                                                  
            dQ_dx = dQG_da(j,i,l+1,1)
            dQ_dy = dQG_da(j,i,l+1,2)
            dQ_dz = dQG_da(j,i,l+1,3)

            efx(i) = efx(i) - dQ_dx*(pot_self(l+1))
            efy(i) = efy(i) - dQ_dy*(pot_self(l+1))
            efz(i) = efz(i) - dQ_dz*(pot_self(l+1))
                                                                                                  
          End Do
        Else If (alf(i,j) <= natms) Then
          ...
          efx(alf(i,j)) = efx(alf(i,j)) + dQ_dx*(pot_self(1))
          efy(alf(i,j)) = efy(alf(i,j)) + dQ_dy*(pot_self(1))
          efz(alf(i,j)) = efz(alf(i,j)) + dQ_dz*(pot_self(1))

          !Dipole-charge + dipole-dipole (self)
          Do l=1,3
            ...
            efx(alf(i,j)) = efx(alf(i,j)) - dQ_dx*(pot_self(l+1))
            efy(alf(i,j)) = efy(alf(i,j)) - dQ_dy*(pot_self(l+1))
            efz(alf(i,j)) = efz(alf(i,j)) - dQ_dz*(pot_self(l+1))
        ...
        Else
          ...
          Do k=1,mpi_counter
            If (alf(i,j) .EQ. mpi_list(k)) Then
              ...
              fxc(k) = fxc(k) + dQ_dx*(pot_self(1))
              fyc(k) = fyc(k) + dQ_dy*(pot_self(1))
              fzc(k) = fzc(k) + dQ_dz*(pot_self(1))
              ...
              Do l=1,3
                ...
                fxc(k) = fxc(k) - dQ_dx*(pot_self(l+1))
                fyc(k) = fyc(k) - dQ_dy*(pot_self(l+1))
                fzc(k) = fzc(k) - dQ_dz*(pot_self(l+1))
      ...
      Do j=1,model_list(ltg(i))%ptr%natms_model-3
        ...
        If (ind <= natms) Then
          ...
          !Charge-charge + charge-dipole + self
          dQ_dx = dQG_da(j+3,i,1,1)
          dQ_dy = dQG_da(j+3,i,1,2)
          dQ_dz = dQG_da(j+3,i,1,3)
                                                                  
          efx(ind) = efx(ind) + dQ_dx*(pot_self(1))
          efy(ind) = efy(ind) + dQ_dy*(pot_self(1))
          efz(ind) = efz(ind) + dQ_dz*(pot_self(1))
                                                                                                  
          !Dipole-charge + dipole-dipole 
          Do l=1,3

            dQ_dx = dQG_da(j+3,i,l+1,1)
            dQ_dy = dQG_da(j+3,i,l+1,2)
            dQ_dz = dQG_da(j+3,i,l+1,3)
                                   
            efx(ind) = efx(ind) - dQ_dx*(pot_self(l+1))
            efy(ind) = efy(ind) - dQ_dy*(pot_self(l+1))
            efz(ind) = efz(ind) - dQ_dz*(pot_self(l+1))
  ...
  Else If (mxompl == 2) Then
    ...
    Do i=1,natms
      ...
      pot_self_cq = -2.0_wp*twzz*(mplgfr(5,i)+mplgfr(8,i)+mplgfr(10,i))

      pot_self(5) = -2.0_wp*((twzz*mplgfr(1,i))+(twtwz*(mplgfr(8,i)+mplgfr(10,i)))+(fozz*mplgfr(5,i)))
      pot_self(6) = -2.0_wp*twtwz*mplgfr(6,i)
      pot_self(7) = -2.0_wp*twtwz*mplgfr(7,i)
      pot_self(8) = -2.0_wp*((twzz*mplgfr(1,i))+(twtwz*(mplgfr(5,i)+mplgfr(10,i)))+(fozz*mplgfr(8,i)))
      pot_self(9) = -2.0_wp*twtwz*mplgfr(9,i)
      pot_self(10)= -2.0_wp*((twzz*mplgfr(1,i))+(twtwz*(mplgfr(5,i)+mplgfr(8,i)))+(fozz*mplgfr(10,i)))

      Do j=1,3
    
        If (alf(i,j) .EQ. i) Then
                                                                                                  
          !Charge-charge + charge-quadrupole 
          dQ_dx = dQG_da(j,i,1,1)
          dQ_dy = dQG_da(j,i,1,2)
          dQ_dz = dQG_da(j,i,1,3)
                                                                                                  
          efx(i) = efx(i) + dQ_dx*(pot_self(1)-pot_self_cq)
          efy(i) = efy(i) + dQ_dy*(pot_self(1)-pot_self_cq)
          efz(i) = efz(i) + dQ_dz*(pot_self(1)-pot_self_cq)
          ...
          !Quadrupole-charge + quadrupole-quadrupole
          Do l=4,9
        ...
        Else If (alf(i,j) <= natms) Then
          ...
          efx(alf(i,j)) = efx(alf(i,j)) + dQ_dx*(pot_self(1)-pot_self_cq)
          efy(alf(i,j)) = efy(alf(i,j)) + dQ_dy*(pot_self(1)-pot_self_cq)
          efz(alf(i,j)) = efz(alf(i,j)) + dQ_dz*(pot_self(1)-pot_self_cq)
        ...
        Else
          ...
          Do k=1,mpi_counter
            If (alf(i,j) .EQ. mpi_list(k)) Then
              ...
              fxc(k) = fxc(k) + dQ_dx*(pot_self(1)-pot_self_cq)
              fyc(k) = fyc(k) + dQ_dy*(pot_self(1)-pot_self_cq)
              fzc(k) = fzc(k) + dQ_dz*(pot_self(1)-pot_self_cq)
      ...
      Do j=1,model_list(ltg(i))%ptr%natms_model-3
        ...
        If (ind <= natms) Then
          ...
          efx(ind) = efx(ind) + dQ_dx*(pot_self(1)-pot_self_cq)
          efy(ind) = efy(ind) + dQ_dy*(pot_self(1)-pot_self_cq)
          efz(ind) = efz(ind) + dQ_dz*(pot_self(1)-pot_self_cq)
...
! For higher order contributions to the stress tensor
             ...
             !qti_local arrays are the Q_beta() arrays in Boateng.
             !pterm = 2S(k)
...
! calculate atomic forces
  ...
  Call spme_mforces(rcell,scale, ixx,iyy,izz,bsddx,bsddy,bsddz, qqc_local,ixb,ixt, iyb,iyt, izb,izt,dQG_da,efx,efy,efz,mpi_counter,fxc,fyc,fzc)
...
Contains
  ...
  Subroutine spme_mforces(rcell,scale, ixx,iyy,izz, bsddx,bsddy,bsddz, qqc_local,ixb,ixt, iyb,iyt, izb,izt,dQG_da,efx,efy,efz,mpi_counter,fxc,fyc,fzc)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    Use config_module, Only : fxx,fyy,fzz,ltg

    Implicit None

    Integer,           Intent( In    ) :: ixx(1:mxatms),iyy(1:mxatms),izz(1:mxatms), &
                                          ixb,ixt, iyb,iyt, izb,izt, mpi_counter
    Real( Kind = wp ), Intent( In    ) :: scale,rcell(1:9),                &
                                          ...
                                          dQG_da(max_atms_model,nlast,mximpl,3)
    Real( Kind = wp ), Intent( InOut ) :: efx(natms),efy(natms),efz(natms), &
                                        & fxc(mpi_counter),fyc(mpi_counter), &
                                        & fzc(mpi_counter)

    Integer           :: fail(1:3), delspl, ixdb,iydb,izdb,ixdt,iydt,izdt, &
                         i,j,k,l, jj,kk,ll
...
! Define extended ranges for the domain = local + halo slice and allocate
    ...
    End If
    Allocate (dqqc( ixdb:ixdt, iydb:iydt, izdb:izdt, 1:3 ), Stat = fail(3))
    If (Any(fail > 0)) Then
       Write(nrite,'(/,1x,a,i0)') 'fflux spme_mforces allocation failure, node: ', idnode
    End If
...
!  !$OMP& PRIVATE(i,imp,imp1,imp2,imp3,imp4,imp5,imp6,imp7,imp8,imp9,imp10) &
!  !$OMP& PRIVATE(fix,fiy,fiz,l,ll,k,kk,j,jj,bdz,bdy,bdx,qsum,dtp,tq2,tq3,tq4) &
!  !$OMP& PRIVATE(tq5,tq6,tq7,tq8,tq9,tq10) &
!  !$OMP& SHARED(natms,mplgfr,mxompl,induce,zero_plus,indipx,indipy,indipz,tmp) &
!  !$OMP& SHARED(mxspl,ixx,iyy,izz,bsddz,bsddy,bsddx,fxx,fyy,fzz,qqc_domain,ka11) &
!  !$OMP& SHARED(kb22,kc33,ka11sq,kakc,kbkc,kakb,kb22sq,kc33sq,ka11cu,kasqkb) &
!  !$OMP& SHARED(kasqkc,kakbsq,kakbkc,kakcsq,kb22cu,kbsqkc,kbkcsq,kc33cu,l_cp) &
!  !$OMP& SHARED(fcx,fcy,fcz,ltg) &
!  !$OMP& REDUCTION(+:fff)
...
! get the components for site i infinitesimal rotations
          ...
          Call fflux_spme_mforces_d(i,qqc_domain,dqqc,bsddx,bsddy,bsddz,ixx,iyy,izz,ixdb,ixdt,iydb,iydt,izdb,izdt,ka11,kb22,kc33,ka11sq,kakb,kakc,kb22sq,kbkc,kc33sq,dQG_da,fqx,fqy,fqz,scale,fff,efx,efy,efz,mpi_counter,fxc,fyc,fzc)
