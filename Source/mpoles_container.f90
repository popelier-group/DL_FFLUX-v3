...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  Use kinds_f90
  Use setup_module, Only : mxompl, sqrpi
...
Subroutine explicit_ewald_real_loops         &
           ...
           enempol,fx,fy,fz,iatm,jatm,scl,   &
           dQG_da,fac,fix,fiy,fiz,kx,efx,efy,efz)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  Use kinds_f90
  Use setup_module,  Only : mxompl,mximpl
  Use mpoles_module, Only : mplmap
  Use fflux_module,  Only : poleOrder,model_list,alf,non_alf_atms, &
                          & Uni_Ind,max_atms_model
  Use ewald_module,  Only : alpha_pow
  Use config_module, Only : fxx,fyy,fzz,ltg,lsa,lsi,nlast,natms

  Implicit None

  Integer,          Intent( In    ) :: lbl,torderlim,k1,k2,k3,iatm,jatm
  ...
  Real( Kind = wp), Intent( InOut ) :: enempol,fx,fy,fz,fix,fiy,fiz
  Real( Kind = wp), Intent( InOut ) :: efx(natms),efy(natms),efz(natms)

! Local variables

  Integer          :: ii,n,s1,s2,s3,ks1,ks2,ks3,ks11,ks21,ks31, &
                    & jind,iind,ind,h,local_index
  Real( Kind = wp) :: tmp,tmpi,tmpj,t1,alphan,td,dQ_dx,dQ_dy,dQ_dz, &
                    & potential,faci,facj,potentiali

  If (lbl /= 0 .and. lbl /= -2) STOP ! Allowed Lower_Bound_Limit

  jind = Uni_Ind(k1,k2,k3)
  facj = scl*fac*jmp
  faci = kx*fac
...
!FFLUX force
     ...
     !ALF iatm
     iind = Uni_Ind(s1,s2,s3)
     potential = tmp*facj

     dQ_dx = dQG_da(1,iatm,iind,1)
     dQ_dy = dQG_da(1,iatm,iind,2)
     dQ_dz = dQG_da(1,iatm,iind,3)

     efx(iatm) = efx(iatm) - potential*dQ_dx
     efy(iatm) = efy(iatm) - potential*dQ_dy
     efz(iatm) = efz(iatm) - potential*dQ_dz

     Do h=2,3
       If (alf(iatm,h) <= natms) Then
         dQ_dx = dQG_da(h,iatm,iind,1)
         dQ_dy = dQG_da(h,iatm,iind,2)
         dQ_dz = dQG_da(h,iatm,iind,3)

         efx(alf(iatm,h)) = efx(alf(iatm,h)) - potential*dQ_dx
         efy(alf(iatm,h)) = efy(alf(iatm,h)) - potential*dQ_dy
         efz(alf(iatm,h)) = efz(alf(iatm,h)) - potential*dQ_dz
       End If
     End Do

     !NON-ALF iatm
     Do h=1,model_list(ltg(iatm))%ptr%natms_model-3

       ind = local_index(non_alf_atms(ltg(iatm),h), nlast,lsi,lsa)

       If (ind <= natms) Then
         dQ_dx = dQG_da(h+3,iatm,iind,1)
         dQ_dy = dQG_da(h+3,iatm,iind,2)
         dQ_dz = dQG_da(h+3,iatm,iind,3)

         efx(ind) = efx(ind) - potential*dQ_dx
         efy(ind) = efy(ind) - potential*dQ_dy
         efz(ind) = efz(ind) - potential*dQ_dz
       End If

     End Do

     !ALF jatm
     potentiali = faci*tmp*imp(ii)
...
! energy
     ...
     !ALF jatm
     potentiali = potentiali + faci*tmp*imp(ii)
...
! energy
     ...
     If (mxompl == 3) Then
       Do h=1,3
         If ((jatm <= natms) .AND. (alf(jatm,h) <= natms)) Then

           dQ_dx = dQG_da(h,jatm,jind,1)
           dQ_dy = dQG_da(h,jatm,jind,2)
           dQ_dz = dQG_da(h,jatm,jind,3)

           If (alf(jatm,h) .EQ. iatm) Then
             efx(iatm) = efx(iatm) - potentiali*dQ_dx
             efy(iatm) = efy(iatm) - potentiali*dQ_dy
             efz(iatm) = efz(iatm) - potentiali*dQ_dz
           Else
             efx(alf(jatm,h)) = efx(alf(jatm,h)) - potentiali*dQ_dx
             efy(alf(jatm,h)) = efy(alf(jatm,h)) - potentiali*dQ_dy
             efz(alf(jatm,h)) = efz(alf(jatm,h)) - potentiali*dQ_dz
           End If
       ...
       !NON-ALF jatm
       Do h=1,model_list(ltg(jatm))%ptr%natms_model-3

         ind = local_index(non_alf_atms(ltg(jatm),h), nlast,lsi,lsa)

         If ((jatm <= natms) .AND. (ind <= natms)) Then
           dQ_dx = dQG_da(h+3,jatm,jind,1)
           dQ_dy = dQG_da(h+3,jatm,jind,2)
           dQ_dz = dQG_da(h+3,jatm,jind,3)
           ...
           If (ind .EQ. iatm) Then
             ...
             efz(iatm) = efz(iatm) - potentiali*dQ_dz
           Else
             efx(ind) = efx(ind) - potentiali*dQ_dx
             efy(ind) = efy(ind) - potentiali*dQ_dy
             efz(ind) = efz(ind) - potentiali*dQ_dz
           End If
         End If
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  Use kinds_f90
  Use setup_module,  Only : mxompl,mximpl,mxspl
  Use mpoles_module, Only : mplmap
  Use ewald_module
...
! Is there no torque when the bond angle is pi?
     ...
     If (numnbh > 2) Then
        ...
        If (fail > 0) Then
           Write(nrite,'(/,1x,a,i0)') 'allocation failure in rotate_mpoles, node: ', idnode
           Call error(0)
        End If
        ...
        If (fail > 0) Then
           Write(nrite,'(/,1x,a,i0)') 'deallocation failure in rotate_mpoles, node: ', idnode
           Call error(0)
        End If
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  Use kinds_f90
  Use mpoles_module, Only : mplltg,mplgfr,mprotx,mproty,mprotz
  Use setup_module,  Only : mxompl,mximpl
...
Subroutine rotate_mpoles_d(iatm)
...
! Is there no torque when atom has only one bond?
     ...
     If (numnbh == 2) Then
        ...
        !True unless angle between p1,p2 is 0,+-pi,+-2pi etc.
        If ((p1x*p2x+p1y*p2y+p1z*p2z)**2 < rrp12*rrp22 - 1.0e-6_wp) Then
...
! Is there no torque when the bond angle is pi?
     ...
     If (numnbh > 2) Then
        ...
        Allocate (xxt(1:numnbh),yyt(1:numnbh),zzt(1:numnbh),rsqt(1:numnbh), Stat=fail)
        If (fail > 0) Then
           Write(nrite,'(/,1x,a,i0)') 'allocation failure in rotate_mpoles_d, node: ', idnode
           Call error(0)
        End If
        ...
        Deallocate (xxt,yyt,zzt,rsqt, Stat=fail)
        If (fail > 0) Then
           Write(nrite,'(/,1x,a,i0)') 'deallocation failure in rotate_mpoles_d, node: ', idnode
           Call error(0)
        End If
     ...
     !BCBS bug fix
     mprotm(iatm)%mtrxa(7) = a7 ; mprotm(iatm)%mtrxa(8) = a8 ; mprotm(iatm)%mtrxa(9) = a9
