...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  Use fflux_module,   Only : model_names
  ...
  Integer                :: fail(1:4),i,j,idm,max_fail,min_fail, &
                            ...
                            ix,iy,iz,jx,jy,jz,ffluxCounter
...
! Define always read serial - BCBS makes life easier for FFLUX MPI.
...
! Read in transmission arrays
        ...
        If (idnode == 0 .and. safe) Then
           ...
           If (l_ind) Then
              ...
              !Read in FFLUX model name - 05/20 BCBS
              Call get_word(record,word)
              model_names(ffluxCounter) = word
              ffluxCounter = ffluxCounter + 1
...
!           Call pbcshift(imcon,cell,indatm,axx,ayy,azz)
           ...
           If (mxnode > 1) Then
              ...
              !Broadcast FFLUX model names - 05/20 BCBS
              Call MPI_BCAST(model_names,megatm*64,MPI_CHARACTER,0,dlp_comm_world,ierr)
