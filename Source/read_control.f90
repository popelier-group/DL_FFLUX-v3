Subroutine read_control                                &
           ...
           ndump,pdplnc,timjob,timcls,poleOrder)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  Use fflux_module,    Only : ewaldFlag,lfflux,nsfflux,isfflux,   &
                              ldipole,nsdipole,isdipole,cutDipole, &
                              cutQuad,lEleConv,clustFlag,lfflux_energy, &
                              lfflux_force, fflux_convergence_criteria, &
                              fflux_check_convergence, set_Force_Max_Thresh, &
                              set_Force_RMS_Thresh, set_Ener_Max_Thresh, &
                              set_Dis_Max_Thresh, set_Dis_RMS_Thresh
...
! default switch for FFLUX outputting (always true) and default for
! (i) step to start at, (ii) every step after to be collected.
! Added by BCBS 03/2019

  lfflux = .false.
  nsfflux = 0
  isfflux = 1

! Switch for printing atomic IQA energies. (FFLUX)

  lfflux_energy = .false.

! Switch for printing atomic IQA forces. (FFLUX)

  lfflux_force = .false.

!FFLUX Dipole and quadrupole terms cut-off radii - BCBS 5/20

  cutDipole = 0.0_wp
  cutQuad = 0.0_wp 

!FFLUX Electrostatic convergence test logical - BCBS 8/20

  lEleConv = .false.

! default switch for FFLUX dipole outputting and defaults for 
! (i) step to start at, (ii) every step after to be collected.
! Added by BCBS 03/2019
...
! read real space cutoff
     ...
     Else If (word(1:3) == 'cut' .or. word(1:4) == 'rcut') Then
        ...
        !Default fflux cut off radii set here - BCBS 5/20
        cutDipole = rcut1
        cutQuad   = rcut1
...
! read geometry optimisation convergence check options
      Else If (word(1:8) == "converge") Then
         fflux_check_convergence = .TRUE.
      Else If (word(1:8) == "criteria") Then
         Call get_word(record, word)
         fflux_convergence_criteria = Abs(Nint(word_2_real(word, 1.0_wp)))
 
      Else If (word(1:10) == "max_energy") Then
         Call get_word(record, word)
         set_Ener_Max_Thresh = word_2_real(word, 1.0_wp)
      Else If (word(1:9) == "max_force") Then
         Call get_word(record, word)
         set_Force_Max_Thresh = word_2_real(word, 1.0_wp)
      Else If (word(1:9) == "rms_force") Then
         Call get_word(record, word)
         set_Force_RMS_Thresh = word_2_real(word, 1.0_wp)
      Else If (word(1:8) == "max_disp") Then
         Call get_word(record, word)
         set_Dis_Max_Thresh = word_2_real(word, 1.0_wp)
      Else If (word(1:8) == "rms_disp") Then
         Call get_word(record, word)
         set_Dis_RMS_Thresh = word_2_real(word, 1.0_wp)

! read coulombic potential option for fflux forces 
! Updated to include ewald and L options -- 11/2018 BCBS

     Else If (word(1:5) == 'fflux') Then

        Call get_word(record,word)

        If((word(1:1) == 'L') .OR. (word(1:1) == 'l')) Then
        
            keyfce = 7
            poleOrder = word_2_real(word(2:2))
            If (lforc) Call error(416)
            lforc=.TRUE.
        !FFLUX ewald option
        ...
        !FFLUX cluster option
        Else If (word(1:7) == 'cluster') Then                      
         ...
         !FFLUX electrostatic convergence test option.
         Else If (word(1:4) == 'conv') Then
 
             lEleConv = .TRUE.

       !fflux variable electrostatics cut-off radius options. - BCBS 5/20
        Else If (word(1:3) == 'cut') Then
        
            Call get_word(record,word)

            If (word(1:6) == 'dipole') Then

                Call get_word(record,word)
                cutDipole = word_2_real(word(:))

            Else If (word(1:10) == 'quadrupole') Then

                Call get_word(record,word)
                cutQuad = word_2_real(word(:))

            End If

       !fflux print start/frequency options - BCBS 03/2019 
        Else If (word(1:5) == 'print') Then

            Call get_word(record,word)                                                    
            nsfflux = Abs(Nint(word_2_real(word)))
            
            Call get_word(record,word)
            isfflux = Abs(Nint(word_2_real(word)))
                                                                               
            If (idnode == 0) Write(nrite,"(/,1x,'FFLUX file start',10x,i10, & 
               & /,1x,'FFLUX file interval '10x,i10)") nsfflux,isfflux

        !IQA energy print option
        Else If (word(1:6) == 'energy') Then
          lfflux_energy = .true.
        !IQA force print option
        Else If (word(1:5) == 'force') Then
          lfflux_force = .true.
        End If

        If (idnode == 0) Write(nrite,"(/,1x,'Electrostatics : Vcl(A,A')')")
...
! read print options
     ...
     Else If (word(1:5) == 'print') Then
        ...
        Else If (word(1:3) == 'vaf') Then
           lpvaf = .true.
        !FFLUX print dipole option - BCBS 03/2019
        Else If (word(1:6) == 'dipole') Then
           ldipole = .true.
        Else
...
! read dipole printing option - BCBS 03/2019

     Else If (word(1:6) == 'dipole') Then

        Call get_word(record,word)
        nsdipole = Abs(Nint(word_2_real(word)))

        Call get_word(record,word)
        isdipole = Abs(Nint(word_2_real(word)))

        If (idnode == 0) Write(nrite,"(/,1x,'FFLUX dipole file start ',10x,i10, &
           & /,1x,'FFLUX dipole file interval '10x,i10)") nsdipole,isdipole
