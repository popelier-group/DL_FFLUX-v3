...
! initialize derivatives
  ...
  bsddx(:,:,natms:nlast)=0.0_wp
  bsddy(:,:,natms:nlast)=0.0_wp
  bsddz(:,:,natms:nlast)=0.0_wp
...
!!$OMP& SHARED(natms,xxx,yyy,zzz,nospl,bspx,bspy,bspz,pmo_no,ncombk) &
!!$OMP& SHARED(bsddx,bsddy,bsddz,real_no,inv_no) &
!!$OMP& PRIVATE(i,j,k,p,m,n,tempx,tempy,tempz,r,s,pcombr,rix0,riy0,riz0) &
!!$OMP& PRIVATE(k_r,km1_rr,jm1_r,aaa,bbb,ccc)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  Use mpoles_module, Only : ncombk
  Use ewald_module,  Only : ka11_pow,kb22_pow,kc33_pow,kprod
...
!     Dtpbsp = ka11**s1 * kb22**s2 * kc33**s3 * bsddx(s1) * bsddy(s2) * bsddz(s3)
     Dtpbsp = kprod(s1,s2,s3) * bsddx(s1) * bsddy(s2) * bsddz(s3)
