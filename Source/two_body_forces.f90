...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  Use config_module,  Only : volm,sumchg,natms,list,xxx,yyy,zzz,atmnam,chge,fyy,fxx,fzz,ltg,nlast,vxx,vyy,vzz,weight
  ...
  Use fflux_module, Only : maxFeatures,ewaldFlag,nsfflux,isfflux,mpi_list
  ...
  Integer           :: fail,i,j,k,limit,keyfce,jcrt_c,i_traj,callFlag,ind
  Real( Kind = wp ) :: engcpe_rc,vircpe_rc,engcpe_rl,vircpe_rl, &
                       ...
                       engacc,viracc,tmp,buffer(0:18),Q,engke,  &
                       engketemp

  Real( Kind = wp ), Dimension( : ), Allocatable :: xxt,yyt,zzt,rrt,dQ_df
  Character (LEN =225) :: QQQ,printchar

!ZEH 21/11/17
  Real( Kind = wp ) :: chgea, qsum
!ZEH 

  keyfce=7 ! JCRT Added this
  Allocate(dQ_df(maxFeatures))
  Allocate(mpi_list(natms))
...
! calculate coulombic forces, Ewald sum - fourier contribution
  ...
  If (keyfce == 2 .and. l_fce) Then
     If (mximpl > 0) Then
        ...
        End If
     Else
        Call ewald_spme_forces(alpha,epsq,engcpe_rc,vircpe_rc,stress,callFlag)
     End If
  End If
...
!Calls to main FFLUX routines.
 If (keyfce == 7) Then

    Call fflux_calc_forces(engmet,virmet,engcpe_rl,vircpe_rl,xxt,yyt,zzt,stress,rcut,epsq,nstep)

    If (ewaldFlag) Call fflux_ewald(rcut,alpha,epsq,engcpe_rc,vircpe_rc,engcpe_rl,vircpe_rl,engcpe_ex,vircpe_ex,stress,keyens,megfrz,nsteql,nstfce,nstep,rlnk)
...
! Added by M.N. to calculate kinetic energy 04/2023
  !$OMP PARALLEL DEFAULT(NONE) &
  !$OMP& SHARED(weight, vxx, vyy, vzz, natms) &
  !$OMP& PRIVATE(i)&
  !$OMP& REDUCTION(+:engkeTemp)
    !$OMP DO
      Do i = 1, natms
        engkeTemp = engkeTemp + weight(i)*(vxx(i)*vxx(i)+vyy(i)*vyy(i)+vzz(i)*vzz(i))
      End Do
    !$OMP END DO
  !$OMP END PARALLEL
  engke = 0.5_wp*engkeTemp  
...
! FFLUX printing options - BCBS 03/2019
  If ((nstep .GE. nsfflux) .AND. (Modulo(nstep,isfflux) == 0) .AND. (idnode .EQ. 0)) Then
    Write(nffluxdt,'(i10,4f20.10)') nstep, engmet/262550_wp, engvdw/100_wp, engcpe/100_wp,&
    &engke/100_wp 
  End If
